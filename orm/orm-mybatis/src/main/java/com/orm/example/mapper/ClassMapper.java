...
import com.orm.example.entity.IClass;
...
//    @Select("select * from class limit #{pageNo}*#{pageSize},#{pageSize}")
    List<IClass> selectList(@Param("startNo") int startNo, @Param("pageSize") int pageSize);
    ...
    @Insert("insert into class values(#{no},#{name},#{stuNum})")
    Integer insertClass(IClass cls);
    @Update("update class set class_name = #{name} where no= #{no}")
